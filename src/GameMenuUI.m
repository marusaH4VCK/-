...
+ (instancetype)sharedButton {
    if (!_sharedFloatingButton) {
        _sharedFloatingButton = [[GMFloatingButton alloc] init];
    }
...
- (void)install {
    ...
    if (@available(iOS 13.0, *)) {
        for (UIScene *sc in [UIApplication sharedApplication].connectedScenes) {
            if ([sc isKindOfClass:[UIWindowScene class]]) {
                _window = [[_GMFloatWindow alloc] initWithWindowScene:(UIWindowScene *)sc];
                break;
            }
        }
    }
    ...
    _btnView.layer.shadowColor   = [UIColor blackColor].CGColor;
    _btnView.layer.shadowOpacity = 0.50f;
    _btnView.layer.shadowRadius  = 10.0f;
    ...
    _ring.layer.borderColor   = [UIColor colorWithWhite:1.0f alpha:0.70f].CGColor;
    ...
    if (imgPath) {
        ...
        _imgView.image = [UIImage systemImageNamed:@"scope"];
    ...
    _dot.layer.shadowOpacity = 0.30f;
...
- (void)_panned:(UIPanGestureRecognizer *)gr {
    if (gr.state == UIGestureRecognizerStateBegan) {
        _dragStart   = [gr locationInView:_window];
        _originStart = _btnView.frame.origin;
        _didDrag     = NO;
        [UIView animateWithDuration:0.12 animations:^{
            self->_btnView.transform = CGAffineTransformMakeScale(1.08f, 1.08f);
        }];
    } else if (gr.state == UIGestureRecognizerStateChanged) {
        CGPoint cur = [gr locationInView:_window];
        CGFloat dx = cur.x - _dragStart.x;
        CGFloat dy = cur.y - _dragStart.y;
        if (fabs(dx) + fabs(dy) > 5) _didDrag = YES;
        _btnView.frame = CGRectMake(
            _originStart.x + dx,
            _originStart.y + dy,
